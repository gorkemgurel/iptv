...
function open_app(){
	...
	if(vgl=="samsung") loc+='?country=RU&language=21&lang=ru&modelid=Genoa_Yahoo&server=operation&remocon=0_650_259_13&area=PANEURO&product=0&mgrver=3.745';
	...
	url='http://smart.obovse.ru/?do=load&appid='+apps_url[select].replace(/:/g,"").replace(/\//g,""); 
	xhr.open('GET', url, true);
	xhr.send();
...
function showapps(ch){
	...
	for (var index = 0; index < ch.length&&index<3; index++)
	{
		...
		html+='<div onmouseover="select='+index+';list();" id="app'+index+'" onclick="open_app();" class="linkapp" style="height:103px;margin:2px 0px;border-radius:7px;padding:1px;background-color: #293B49;border:2px solid transparent;'+st+';"><img style="float:left;margin-right:5px;border-radius:3px;" width="104" height="100" src="'+x[2]+'" />'+x[0]+'<br><b>'+x[1]+'</b><br>'+x[3]+'</div>';
		apps_url[index]=x[4];
	...
	for (var index = 3; index < ch.length; index++)
	{
		...
		html+='<div onmouseover="select='+index+';list();" id="app'+index+'" onclick="open_app();"  class="linkapp" style="border:2px solid transparent;text-align:center;height:125px;width:180px;margin:0px 0px;border-radius:5px;padding:2px 0px 0px 0px;float:left;'+lft+st+';"><img style="border-radius:3px;" width="80" height="80" src="'+x[2]+'" /><br><b>'+x[1]+'</b><div style="font-size:15px;color:yellow;">'+x[3]+'</div></div>';
...
function list(t,m){
	...
	for(var i=0;i<count;i++){
		if(i>2){
			if(m){
				if(row>2&&i<=(2+((row-2)*6))) $_("app"+i).style.display="none";
